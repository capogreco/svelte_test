...
async function startCtrlClient (
  ...
  pc.onicecandidate = e => {
    if (e.candidate) {
      set (ref (db, `ctrlOffers/${ ctrlId }/c†rlIce`), e.candidate.toJSON ())
  ...
  onValue (ref(db, `ctrlOffers/${ ctrlId }/answer`), async snap => {
    ...
    if (ans) {
      try {
        ...
        console.error('Error settting remote description:', e);
      }
    }
  });

  onValue (ref(db, `ctrlOffers/${ ctrlId }/synthIce`), async snap => {
    console.log ('synthIce onValue triggered !!!!!!!!!!!!!!!!!!!!')
    console.dir (snap)
    const ice = snap.val ()
    console.dir (ice)
    if (ice) {
